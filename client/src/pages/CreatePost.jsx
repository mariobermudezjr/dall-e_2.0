...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch('https://dall-e-qjjk.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true)
      try {
        const response = await fetch('https://dall-e-qjjk.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <section className="max-q-7x1 mx-auto">
      <div>
        <h1 className="font-extra-bold text-[#222328]"> Create</h1>
